...
| H12 | S | ✅ | `10ef694` | worker-pool se souběžností 3 (stejný vzor jako `findDiscoveredDevices`); R9 zachováno – všechny soubory se zpracují i po chybě, chyby se sbírají a logují jednou, navíc seřazené podle pořadí souborů (deterministická diagnostika). Předepsaná změna testu: `maximumActiveUploads` 1 → >1 a ≤3; pořadí uploadů se nezměnilo. 328 pass |
| H13 | O | ✅ | `93b117a` + `5875783` | ESLint 7.32→**8.57.1**, typescript-eslint 5→**8.46** (peer `typescript >=4.8.4 <6.1.0` = TS 5.9 ✓). **ESLint 9 není dosažitelný:** `eslint-config-athom` 3.1.5 je nejnovější a drží ESLint‑7 éru přes `eslint-config-airbnb-base@14` (peer eslint ≤7) a typescript‑eslint 6 → vyřešeno `overrides` jen na tyto tranzitivní piny, athom ruleset nezměněn. Nutný `rm -rf node_modules` + čerstvý lockfile (npm reusoval starý strom). Follow-up commit: 49 nálezů – `import/no-import-module-exports` (43) vypnuto pro Homey entrypointy (musí dual-exportovat), `default-param-last` (2) vypnuto (reorder by změnil veřejné signatury), `no-promise-executor-return` (4) mechanicky opraveno v testech. Bez změny runtime. **Pozn.:** ESLint 8.57.1 je upstream EOL – další krok čeká na nový `eslint-config-athom` |
...
| REL2 | – | ✅ připraveno | `f35516e` | **Publikováno jako 2.2.0** (rozhodnutí bckp – H změnilo pozorovatelné chování, takže ne pokračování nikdy nevydaného 2.1.0). Bump v `.homeycompose/app.json` + `package.json` + `package-lock.json`, `app.json` přegenerován přes `homey app build`. Changelog 2.2.0 (442 znaků) nese dál migrační poznámku k custom app jménům – 2.1.0 se do App Store nedostalo, uživatelé z 2.0.1 by ji jinak nikdy neviděli. `validate --level publish` OK. **Samotný publish provádí bckp** (App Store přihlášení) |
