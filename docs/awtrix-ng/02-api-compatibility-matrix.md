...
| Přehrání RTTTL melodie (`playRTTTL`, `cmdRtttl`) | `POST /api/rtttl` | `POST /api/v1/audio/play` (firmware 1.1.0+) | Raw text `rtttl string`; záměr `Content-Type: text/plain`. | JSON `{ "rtttl": "..." }` s `Content-Type: application/json`. | Status only. | `200 {"ok":true}` nebo error envelope. | Raw text → JSON; key `rtttl`. Verze z posledního úspěšného `GET /api/v1/device` se kontroluje před requestem. | Homey driver nepoužívá starý endpoint jako fallback. Firmware pod 1.1.0 nebo dosud neznámá verze vyvolá `AwtrixNgUnsupportedVersionError`; ostatní funkce zařízení tím nejsou omezené. | Chyba verze vznikne lokálně před requestem; chyby nového endpointu se dál předávají beze změny. | Kompatibilní po transformaci od NG 1.1.0 |
...
| `rtttl` | `soundRtttl` | string → string | ano | ne | Kompatibilní po transformaci | Notification-only; NG notification s nevalidním soundRtttl není rejected podle payload docs, jen nehraje. Standalone `/audio/play` validuje. |
